...
const getAllJobs = async (req, res) => {
  ...
  if (docs.length)
    res.send({
      ...
      categories: ["Concrete Construction", "Consultation", "Finishing Works"],
      services: [
...
const updateJob = async (req, res) => {
  ...
  if (!(await JobModel.findOne({ _id: jobId, userId })))
    throw new CustomError(403, "You do not have right to modify this job!!!");
...
const deleteJob = async (req, res) => {
  ...
  if (!(await JobModel.findOne({ _id: jobId, userId })))
    throw new CustomError(403, "You do not have right to remove this job!!!");
